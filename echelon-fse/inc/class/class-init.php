...
class Init {
	...
	public function maybe_sync_global_styles_after_version_change() {
		$synced_version = get_option( 'echelon-fse_global_styles_synced_version' );

		if ( ECHELON_FSE_VERSION === $synced_version ) {
			return;
		}
	...
	private function sync_global_styles() {
		...
		update_option( 'echelon-fse_global_styles_synced_version', ECHELON_FSE_VERSION );
	}
	...
	public function setup_theme() {
		load_theme_textdomain( 'echelon-fse', get_template_directory() . '/languages' );
	}
  ...
  'name' => 'Fun Fact Super Font (Legacy)',
  'font' => 
  array (
    'font' => 
    array (
  ...
  'id' => 'gv-font-fun-fact-number-font',
  ...
  'id' => 'gv-font-fun-fact-super-font',
  ...
  'id' => 'gv-font-fun-fact-title-font',
  ...
  array (
	...
	public function dashboard_scripts( $hook_suffix ) {
					...
					if ( 'appearance_page_echelon-fse-dashboard' !== $hook_suffix && 'admin_page_gutenverse-onboarding-wizard' !== $hook_suffix ) {
						return;
					}
		...
		$dashboard_includes = include get_template_directory() . '/assets/dependencies/theme-dashboard.asset.php';
					...
					wp_enqueue_style(
						'echelon-fse-dashboard-inter-font',
	...
	public function theme_config() {
		...
		$config = array(
			...
			'logo'          => trailingslashit( get_template_directory_uri() ) . 'assets/img/Logo-Echelon.webp',
			'slug'          => 'echelon-fse',
			'upgradePro'    => esc_url( 'https://gutenverse.com/pricing' ),
			'supportLink'   => esc_url( 'https://wordpress.org/support/theme/echelon-fse/' ),
...
'plus_pattern_count' => 39
			),
			'lite_plus_type' => 'wporg',
			'pro_preview' => trailingslashit( get_template_directory_uri() ) . 'assets/img/ss-cover-echelon-pro-home.webp',
			'pro_title' => esc_html__('Echelon PRO', 'echelon-fse'),
			'upgrade_required_license' => array('agency','enterprise','ultimate','professional'),
		);

		if ( 'themes.php' === $pagenow && isset( $_GET['page'] ) && 'echelon-fse-dashboard' === sanitize_key( wp_unslash( $_GET['page'] ) ) ) {
			$admin_config = array(
		...
		return $config;
						...
						public function system_status() {
							...
							$status['active_companion_demo'] = $active_demo['active_demo'] ?? esc_html__( 'You don\'t have any demo activated', 'echelon-fse' );
						...
						public function data_active_plugin() {
							...
							if ( count( $plugins ) > 0 ) {
								foreach ( $plugins as $plugin ) {
									...
									$item['name']        = isset( $plugin['Name'] ) ? $plugin['Name'] : esc_html__( 'unknown', 'echelon-fse' );
									$item['author_uri']  = isset( $plugin['AuthorURI'] ) ? esc_url( $plugin['AuthorURI'] ) : '#';
									$item['author_name'] = isset( $plugin['Author'] ) ? $plugin['Author'] : esc_html__( 'unknown', 'echelon-fse' );
									$item['version']     = isset( $plugin['Version'] ) ? $plugin['Version'] : esc_html__( 'unknown', 'echelon-fse' );

									$content = esc_html__( 'by', 'echelon-fse' );
						...
						public function admin_menu() {
							add_theme_page(
								esc_html__('Echelon FSE Dashboard', 'echelon-fse'),
								esc_html__('Echelon FSE Dashboard', 'echelon-fse'),
								'edit_theme_options',
								'echelon-fse-dashboard',
