...
class Plugin_Notice {
	...
	private function load_hooks() {
		...
		add_action( 'wp_ajax_echelon-fse_dismiss_notice', array( $this, 'dismiss_notice' ) );
	}
	...
	public function notice_install_plugin() {
		...
		if ( get_option( 'echelon-fse_dismiss_notice' ) ) {
			return;
		}

		$screen = get_current_screen();
		if ( isset( $screen->parent_file ) && 'themes.php' === $screen->parent_file && 'appearance_page_echelon-fse-dashboard' === $screen->id ) {
			return;
		}
  ...
  '1x' => 'https://ps.w.org/gutenverse-companion/assets/icon-128x128.png?rev=3162415',
		...
		wp_register_style( 'echelon-fse-theme-notice', false );
		wp_enqueue_style( 'echelon-fse-theme-notice' );
		ob_start();
		?>
		
				.echelon-fse-simple-notice {
				
					padding: 24px !important; 
				
					border-left: 4px solid #007cba !important;
				}
				.echelon-fse-simple-notice p:first-child {
				
					margin-bottom: 20px; 
					margin-top: 0;
				}
				.echelon-fse-simple-notice p:last-of-type {
				
					margin-top: 0; 
					margin-bottom: 0;
				}
				.echelon-fse-simple-notice .echelon-fse-notice-title {
					font-size: 17px;
					font-weight: 600; 
					display: block;
					margin-bottom: 6px;
				}
				.echelon-fse-simple-notice .echelon-fse-notice-description {
					font-size: 13px;
					display: block;
				...
				.echelon-fse-simple-notice .button-primary {
					padding: 4px 16px !important;
				}

			
				.echelon-fse-simple-notice .notice-dismiss {
					background: none !important;
					box-shadow: none !important;
					opacity: 1;
				}
				
				.echelon-fse-simple-notice .notice-dismiss:before {
					color: #a7aaad;
					padding: 0; 
				}
				
				.echelon-fse-simple-notice .notice-dismiss:hover:before,
				.echelon-fse-simple-notice .notice-dismiss:focus:before {
					color: #c90000;
				}
		...
		wp_add_inline_style( 'echelon-fse-theme-notice', $custom_style );
		wp_register_script(
			'echelon-fse-theme-notice',
			'',
			array( 'wp-api-fetch' ),
			'1.0.0',
			true
		...
		wp_enqueue_script( 'echelon-fse-theme-notice' );
		...
		function sequenceInstall(plugin, pluginsInstalled) {
			return new Promise((resolve, reject) => {
				...
				const needUpdate = plugin.installed
					? ECHELONFSEPluginUtils.isVersionGreater(plugin.req_version, pluginsInstalled[`${path}.php`].Version)
					: false;
		...
		document.addEventListener('DOMContentLoaded', () => {
			const notice = document.querySelector('.notice.is-dismissible.echelon-fse-simple-notice');
				...
				setTimeout(() => {
					...
					if (dismissBtn) {
						dismissBtn.addEventListener('click', (e) => {
							e.preventDefault();
							jQuery.post(ajaxurl, {
								action: 'echelon-fse_dismiss_notice',
								_ajax_nonce: nonce
							});
						});
					}
			...
			button.addEventListener('click', function (e) {
				...
				button.innerHTML = `<?php esc_html_e( 'Installing...', 'echelon-fse' ); ?>`;
				...
				if (!hasFinishClass) {
					...
					sequence.then(() => {
						...
						button.innerHTML = `<?php esc_html_e( 'Install Failed, Try Again', 'echelon-fse' ); ?>`;
		...
		wp_add_inline_script( 'echelon-fse-theme-notice', $custom_script );

		?>
		<div class="notice notice-info is-dismissible echelon-fse-simple-notice" data-nonce="<?php echo esc_attr( wp_create_nonce( "echelon-fse_dismiss" ) ); ?>">
				<p>
					<strong class="echelon-fse-notice-title"><?php esc_html_e( "Thank you For Installing Echelon FSE Theme", "echelon-fse" ); ?></strong>
					<span class="echelon-fse-notice-description">
						<?php esc_html_e( "Unlock the full potential of your website with the recommended plugins.", "echelon-fse" ); ?>
						<br/>
						<?php esc_html_e( "Activate it to explore exclusive extensions, ready-to-use demo templates, and powerful features that make building your site easier and more enjoyable.", "echelon-fse" ); ?>
					</span>
				</p>
				<p>
					<a href="#" class="button button-primary" id="gutenverse-install-plugin"><?php echo esc_html__( "Install Recommended Plugins", "echelon-fse" ); ?></a>
				...
				<p class="installing-warning" style="font-size: 12px; color: #666; display: none; margin-top: 5px;">
					<i><?php esc_html_e( "Don\'t refresh the page when installing recommended plugins", "echelon-fse" ); ?></i>
				</p>
			</div>
		<?php
	}
	...
	public function dismiss_notice() {
		check_ajax_referer( 'echelon-fse_dismiss' );

		if ( ! get_option( 'echelon-fse_dismiss_notice' ) ) {
			update_option( 'echelon-fse_dismiss_notice', true );
		}
